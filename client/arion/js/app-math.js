	...
	.service('mathService', function() {
		...
		var checkBrackets = function(expression) {
			...
			for (let i = 0; i < expression.length; i++) {
				...
				if (counter < 0) {
					throw new Error('A01::mathService.checkBrackets: incorrect expression. excess closing bracket');
				}
			}
			if (counter != 0) {
				throw new Error('A01::mathService.checkBrackets: incorrect expression. excess opening bracket');
			}
			// // console.log('checkBrackets success')
		};

		var checkSafety = function(expression) {
			let reservedWords = ['=', ' ', ';', 'break', 'case', 'constructor', 'delete', 'do', 'else', 'eval', 'finally', 'for',
				'function', 'goto', 'if', 'return', 'prototype', 'try', 'var', 'while'];
			angular.forEach(reservedWords, function(word) {
				if (expression.indexOf(word) != -1) {
					throw new Error('A01::mathService.checkSafety: unsafe expression to be evaluated');
				}
			});
		...
		var replaceVariables = function(expression, data) {
			angular.forEach(data, function(value, key) {
				let replaceThis = new RegExp('(?:\\+|\\-|\\/|\\*|\\(|\\^|^)(' + key + ')(?:\\+|\\-|\\/|\\*|\\)|\\^|$)');
				let matchArray;
				while (matchArray = replaceThis.exec(expression)) {
					...
					let begin = expression.substring(0, beginSubIndex);
					let end = expression.substring(beginSubIndex);
		...
		let replacePower = function(expression, data) {
			// // console.log("innerExpression : %s", expression)
			while (expression.indexOf('^') != -1) {
				let template = expression.match('((?:\\(?-?([0-9]+([\.][0-9]+)?e\-?[0-9]+)\\)?|\\(?-?[0-9]+[\\.]{0,1}(?:[0-9]*)\\)?|\\(?-?[a-zA-Z0-9\\.]+\\)?)(\\^)(?:\\(?-?([0-9]+([\.][0-9]+)?e\-?[0-9]+)\\)?|\\(?-?[0-9]+[\\.]{0,1}(?:[0-9]*)\\)?|\\(?-?[a-zA-Z0-9\\.]+\\)?))');
				...
				try {
					...
					if (err == 'A01::mathService.checkBrackets: incorrect expression. excess opening bracket') {
						let trimmed = removeOpenBracket(template[0]);
						let replacement = trimmed.split('^');
						let powerStr = 'Math.pow(' + replacement[0] + ',' + replacement[1] + ')';
						let result = eval(powerStr);
					...
					if (err == 'A01::mathService.checkBrackets: incorrect expression. excess closing bracket') {
						let trimmed = removeCloseBracket(template[0]);
						let replacement = trimmed.split('^');
						let powerStr = 'Math.pow(' + replacement[0] + ',' + replacement[1] + ')';
						let result = eval(powerStr);
						expression = expression.substring(0, template.index) +
				...
				if (template[0].indexOf('(') == 0 && template[0].indexOf(')') == template[0].length - 1
					...
					replacement = (template[0].substring(1, template[0].length - 1)).split('^');
				}
				// handling 'xxxx^xxxx' case
				else {
					replacement = template[0].split('^');
				}
			...
			for (let i = 0; i < expression.length; i++) {
				...
				if (expression.indexOf('(') > -1) {
					...
					if (expression[i] == ')' && !isClosed) {
						...
						let innerExpression = expression.substring(openBracketPos, i + 1);
						...
						if (expPre) {
							...
							replacement = eval(innerExpression);
							temp = expression.substring(0, openBracketPos - 8)
								+ replacement
								+ expression.substring(i + 1);
						}
						if (!expPre && !lnPre) {
							replacement = eval(innerExpression);
							temp = expression.substring(0, openBracketPos)
								+ replacement
								+ expression.substring(i + 1);
						}
	...
	.service('calculateService', ['mathService', function(mathService) {
		...
		var getPropertyValue = (p) => {
			if (p.Type === undefined || p.Type == 1 || p.Type == 2) { // non-strict equality caused by type differences
				/* debug*/ if (!p.Key || p.value === undefined) {
					throw new Error(`getPropertyValue error : ${p}`);
				}
		...
		var stringToNumber = (str) => {
			let ret = parseFloat(str.replace(',', '.'));
		...
		this.calculateCoefficients = function(element, keysArray) {
			...
			let varObj = keysArray.reduce((acc, item) => {
			...
			let i = 0;
			let calculatedCoefficientsKeys = [];
